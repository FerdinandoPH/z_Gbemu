...
class Cpu:
    def __init__(self, memory, debug_level=0):
        ...
        self.int_names = ["JOYPAD", "SERIAL", "TIMER", "STAT", "VBLANK"]
        #region cycles
        ...
        #endregion
        #region opcode_dict
        self.opcode_table={0x0: self.NOP, 0x10: self.STOP, 0x37: self.SCF, 0x2F: self.CPL, 0x3F:self.CCF, 0x27:self.DAA, 0xCB:self.cb_prefix, **{i:self.di_ei for i in {0xF3, 0xFB}}, **{i:self.LD_8 for i in set(range(2,64,4)) | set(range(0x40,0x80)) - {0x76} | {0xE0, 0xE2, 0xEA, 0xF0, 0xF2, 0xFA}}, **{i:self.pc_change for i in {0x18, 0x20, 0x28, 0x30, 0x38} | {0xC2, 0xC3, 0xCA, 0xD2, 0xDA, 0xE9} | {0xC4, 0xCC, 0xCD, 0xD4, 0xDC} | {0xC0, 0xC8, 0xC9, 0xD0, 0xD8, 0xD9} | set(range(0xC7, 0x100, 0x8))}, 0x76:self.HALT, **{i:self.empty_opcode for i in {0xD3, 0xDB, 0xDD, 0xE3, 0xE4, 0xEB, 0xEC, 0xED, 0xF4, 0xFC, 0xFD}}, **{i:self.CP for i in set(range(0xB8,0xC0)) | {0xFE}}, **{k:self.inc_dec  for i in range(0x03, 0x43, 0x8) for k in range(i,i+3)}, **{i:self.ld_16 for i in set(range(0x1,0x41,0x10)) | {0x8} | {0xF8, 0xF9}}, **{k:self.push_pop for i in range(0xC1, 0x100,0x10) for k in range(i, i+5, 4)}, **{i:self.shift_rot for i in set(range(0x07,0x20,0x8))}, **{i:self.add_sub for i in set(range(0x80, 0xA0)) | {0xC6, 0xCE, 0xD6, 0xDE}}, **{i:self.add_16 for i in set(range(0x9,0x49, 0x10)) | {0xE8}}, **{i:self.logic_ops for i in set(range(0xA0,0xB8)) | {0xE6, 0xEE, 0xF6}}}
        ...
        self.pc_change_table={"new_pc":{**{i:"e8" for i in set(range(0x20,0x40,0x8)) | {0x18}}, **{i:"a16" for i in set(range(0xC2,0xE2,0x8)) | {0xC3, 0xCD} | set(range(0xC4,0xE4,0x8))}, 0xE9:"HL", **{i:"SP" for i in set(range(0xC0,0xE0,0x8)) | {0xC9, 0xD9}}, **{i:i-0xC7 for i in set(range(0xC7,0x100,0x8))}}, "cond":{**{i: "True" for i in {0x18, 0xC3, 0xCD, 0xE9, 0xC9, 0xD9} | set(range(0xC7,0x100,0x8))}, **{i: "NZ" for i in {0x20, 0xC0, 0xC2, 0xC4}}, **{i: "Z" for i in {0x28, 0xC8, 0xCA, 0xCC}}, **{i: "NC" for i in {0x30, 0xD0, 0xD2, 0xD4}}, **{i: "C" for i in {0x38, 0xD8, 0xDA, 0xDC}}}, "name": {**{i:"JR" for i in set(range(0x20,0x40,0x8)) | {0x18}}, **{i: "JP" for i in set(range(0xC2,0xE2,0x8)) | {0xC3, 0xE9}}, **{i:"CALL" for i in set(range(0xC4,0xE4,0x8)) | {0xCD}}, **{i:"RET" for i in set(range(0xC0,0xE0,0x8)) | {0xC9, 0xD9}}, **{i:"RST" for i in set(range(0xC7,0x100,0x8))}}}
        #endregion
        #region LD_8_dict
        self.ld_8_table={
        ...
        #endregion
        #region inc_dec_dict
        self.inc_dec_table={"target":{0x03:"BC", 0x04:"B", 0x05:"B", 0x0B:"BC", 0x0C:"C", 0x0D:"C", 0x13:"DE", 0x14:"D", 0x15:"D", 0x1B:"DE", 0x1C:"E", 0x1D:"E", 0x23:"HL", 0x24:"H", 0x25:"H", 0x2B:"HL", 0x2C:"L", 0x2D:"L", 0x33: "SP", 0x34:"[HL]", 0x35:"[HL]", 0x3B:"SP", 0x3C:"A", 0x3D:"A"}, "op":{0x03:1, 0x04:1, 0x05:-1, 0x0B:-1, 0x0C:1, 0x0D:-1, 0x13:1, 0x14:1, 0x15:-1, 0x1B:-1, 0x1C:1, 0x1D:-1, 0x23:1, 0x24:1, 0x25:-1, 0x2B:-1, 0x2C:1, 0x2D:-1, 0x34:1, 0x35:-1, 0x3B: -1,0x3C:1, 0x3D:-1}}
        #endregion
        #region ld_16_dict
        ...
        self.push_pop_table={"reg":{0xC1:"BC", 0xC5:"BC", 0xD1:"DE", 0xD5:"DE", 0xE1:"HL", 0xE5:"HL", 0xF1:"AF", 0xF5:"AF"}, "name":{**{i:"POP" for i in range(0xC1, 0xF2, 0x10)}, **{i:"PUSH" for i in range(0xC5, 0xF6, 0x10)}}}
        #endregion
        #region shift_rot_dict
        self.shift_rot_table={"reg":{**{i:"A" for i in set(range(0x07, 0x37, 0x8)) | {0x3F}}, **{i:"B" for i in set(range(0x0, 0x30,0x8)) | {0x38}}, **{i:"C" for i in set(range(0x1, 0x31,0x8)) | {0x39}}, **{i:"D" for i in set(range(0x2, 0x32,0x8)) | {0x3A}}, **{i:"E" for i in set(range(0x3, 0x33,0x8)) | {0x3B}}, **{i:"H" for i in set(range(0x4, 0x34,0x8)) | {0x3C}}, **{i:"L" for i in set(range(0x5, 0x35,0x8)) | {0x3D}}, **{i:"[HL]" for i in set(range(0x6, 0x36,0x8)) | {0x3E}}}, "name_direct":{0x07:"RLCA", 0x0F: "RRCA", 0x17: "RLA", 0x1F: "RRA"}, "name":{**{i:"RLC" for i in range(0x0,0x8)}, **{i:"RRC" for i in range(0x8,0x10)}, **{i:"RL" for i in range(0x10,0x18)}, **{i:"RR" for i in range(0x18,0x20)}, **{i:"SLA" for i in range(0x20,0x28)}, **{i:"SRA" for i in range(0x28,0x30)}, **{i:"SRL" for i in range(0x38,0x40)}}}
        #endregion
        #region add_sub_dict
        ...
        for i in range(4, -1, -1):
            if (ie_flag & (1<<i)) and (if_flag & (1<<i)):
                if self.break_on_interrupt:
                    ...
                    print(f"Interrupt {self.int_names[i]} triggered")
                self.IME = False
                self.mem[0xFF0F] &= ~(1<<i)
    ...
    def empty_opcode(self):
        ...
        print("Empty opcode: "+hex(self.instruction)+ " at "+hex(self.pc-1))
        self.state="QUIT"
    ...
    def pc_change(self):
        ...
        if new_pc == "a16":
            ...
            self.pc = (self.pc + 1) % 0x10000
        if self.get_name: return name+("I" if self.instruction == 0xD9 else "")+((" "+self.pc_change_table["cond"][self.instruction]) if self.pc_change_table["cond"][self.instruction] !="True" else "")+" "+(("$"+("+" if name == "JR" and new_pc>=0 else "")+hex(new_pc)) if new_pc != "HL" and name !="RET" else "HL" if new_pc == "HL" else "")
    ...
    def add_16(self):
        ...
        dest = "HL" if self.instruction != 0xE8 else "SP"
        if dest == "SP":
            src = self.mem[self.pc]
            if src > 127: src -= 256
            self.pc = (self.pc + 1) % 0x10000
        ...
        if dest == "SP": self.sp = (value + src) % 0x10000
